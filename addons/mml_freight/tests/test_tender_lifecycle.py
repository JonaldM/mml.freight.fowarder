...
class TestTenderLifecycle(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.po = cls.env['purchase.order'].create({'partner_id': p.id})
        nzd = cls.env['res.currency'].search([('name', '=', 'NZD')], limit=1) or cls.env.company.currency_id
        cls.tender = cls.env['freight.tender'].create({'po_ids': [(4, cls.po.id)], 'company_id': cls.env.company.id, 'currency_id': nzd.id})
    ...
    def test_cancel(self):
        t = self.env['freight.tender'].create({'po_ids': [(4, self.po.id)], 'company_id': self.env.company.id, 'currency_id': self.env.company.currency_id.id})
        t.action_cancel()
    ...
    def test_auto_select_no_quotes_raises(self):
        t2 = self.env['freight.tender'].create({'po_ids': [(4, self.po.id)], 'company_id': self.env.company.id, 'currency_id': self.env.company.currency_id.id, 'state': 'quoted', 'selection_mode': 'cheapest'})
