"""Post-migration for mml_freight 19.0.1.0.0.
...
Why this is needed:
    ...
    When upgrading an existing Odoo instance to v19, the ir.config_parameter
    entry for 'mml_registry.service.freight' may be absent or stale if the
    module was previously at a lower version, leaving mml.registry returning
    NullService for every call to service('freight').
...
def migrate(cr, version):
    ...
    _logger.info(
        'mml_freight 19.0.1.0.0: re-registered FreightService in mml.registry'
    )
